			...
			<table class="table table-striped table-bordered table-condensed table-hover">
               ...
               @foreach ($datosopchofer as $datoopchofer)
				<tr>
					...
					@else
						<td>{{ $datoopchofer->transporte->nombre}}</td>
						<td>{{ $datoopchofer->fecha}}</td>
						<td>{{ $datoopchofer->montoacumulado}}</td>
						<td>{{ $datoopchofer->montofinal}}</td>
						<td>{{ $datoopchofer->estado}}</td>
						...
						@if($datoopchofer->estado=='ABIERTO')
							<a href="{{url('pagos/transporte/'.$datoopchofer->id.'/pagoefectivotransporte')}}"><input type="button" value="Efectivo" class="btn btn-success">	</a>
						@else
							<a href="{{url('pagos/transporte/'.$datoopchofer->id.'/pagoefectivo')}}"><input type="button" disabled value="Efectivo" class="btn btn-success">	</a>
						@endif


						@if($datoopchofer->estado=='ABIERTO')
							<a href="{{url('pagos/transporte/'.$datoopchofer->id.'/cerrarop')}}"><input type="button" value="Cerrar OP" class="btn btn-danger" onclick="return confirm('Seguro que desea Cerrar la Orden de Pago?');">	</a>
						@else
