...
{   
   ...
   public function pagoefectivotransporte($id){
  
    $datosoptransporte=OrdenPago::where('id',$id)->get();
    $datosoptransporte->each(function($datosoptransporte){
          $datosoptransporte->transporte;
          $datosoptransporte->proveedor;
        });
    $datoscaja=Caja::orderBy('denominacion','ASC')->pluck('denominacion','id');
    return view('pagos.transporte.cargarpagoefectivo')
            ->with('id',$id)
            ->with('datoscaja',$datoscaja)
            ->with('datosoptransporte',$datosoptransporte);
   }
   public function pagoefectivoproveedor($id){
       $datoscaja=Caja::orderBy('denominacion','ASC')->pluck('denominacion','id');
    $datosopchofer=OrdenPago::where('id',$id)->get();
    $datosopchofer->each(function($datosopchofer){
          $datosopchofer->transporte;
...
// PAGO EN EFECTIVO PARA TRANSPORTE
  public function guardarpagoefectivochofer(Request $request){
    ...
    $Movimiento=new MovimientoOPC(request()->except('_token')); // pago en efectivo par atransporte
    ...
    $consultasuma=MovimientoOPC::where('ordendepago_id',$request->ordendepago_id)->sum('importe');//suma de importes en el moviemitnos de ops de choferes
      ...
      $datosmovimientoscajas->descripcion='PAGO A TRANSPORTE EN EFECTIVO';
 ...
 public function guardarpagochequeterceroproveedor(Request $request){
   ...
   public function opproveedores(){
    ...
    public function optransportes(){
        $transportes=Transporte::orderBy('nombre','ASC')->pluck('nombre','id');
        return view('pagos.optransportes')
...
public function generaroptransporte(Request $request){
...
public function cerrarop($id){
    ...
    $datoacumulado=Proveedor::where('id',$datosopchofer[0]->proveedor_id)->orderBy('id','DESC')->limit(1)->get();
    ...
    $datosComprobante->acumulado=$datoacumulado[0]->saldo - $datosopchofer[0]->montoacumulado;
...
public function cerraroptransporte($id){
$date = new \DateTime();
     $datosopchofer=OrdenPago::where('id',$id)->get();
     $actualizarop=OrdenPago::where('id',$id)
                ->update([
    ...
    $datostransporte=Transporte::where('id',$datosopchofer[0]->transporte_id)->get();
    $saldofinal=$datostransporte[0]->saldo+$datosopchofer[0]->montoacumulado;
    $actualizartransporte=Transporte::where('id',$datosopchofer[0]->transporte_id)
                ->update([
                'saldo'=>$saldofinal

                     ]);

    $datoacumulado=Transporte::where('id',$datosopchofer[0]->transporte_id)->orderBy('id','DESC')->limit(1)->get();
    

    $acumulado=Transporte::where('id',$id)->orderBy('id','DESC')->limit(1)->get();
    $datosComprobante=new CtaCteTransporte();
    $datosComprobante->transporte_id=$datosopchofer[0]->transporte_id;
    $datosComprobante->tipocomprobante='ORDEN DE PAGO';
    $datosComprobante->nrocomprobante=$datosopchofer[0]->numero;
    $datosComprobante->fechaemision=$date;
    $datosComprobante->fechavencimiento=$date;
    ...
    $datosComprobante->save();
      ...
      flash::success('Se cerro correctamente la OP'); 
    return view('pagos.imputarchofer')
            ->with('datosopchofer',$datosopchofer);
